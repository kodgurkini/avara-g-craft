// Shape definitions with their SVG paths, colors, and patterns
export interface Pattern {
  type: "lines" | "dots" | "wavy" | "grid";
  color: string;
  size: number;
}

export interface Shape {
  path: string;
  fill: string;
  border: string;
  pattern: Pattern;
  name: string;
}

export const shapes: Shape[] = [
  {
    path: "M450.9 169.7a99.4 99.4 0 0 0-140.6 0A99.4 99.4 0 1 0 169.7 29.1a99.4 99.4 0 0 0 140.6 140.6 99.4 99.4 0 1 0 140.6 0Z M169.7 310.3A99.4 99.4 0 1 0 29.1 169.7a99.4 99.4 0 0 0 140.6 140.6 99.4 99.4 0 1 0 140.6 140.6 99.4 99.4 0 0 0-140.6-140.6Z",
    fill: "#ffb7b2",
    border: "#ff8c8c", 
    pattern: {
      type: "lines",
      color: "#ffcbca",
      size: 32,
    },
    name: "Clover",
  },
  {
    path: "M480 240a160 160 0 0 0-240-138.6V0a160 160 0 0 0-138.6 240H0a160 160 0 0 0 240 138.6V480a160 160 0 0 0 138.6-240H480Z",
    fill: "#c1f7c8",
    border: "#7bda88", 
    pattern: {
      type: "wavy",
      color: "#d1ffd7",
      size: 1,
    },
    name: "Pinwheel",
  },
  {
    path: "m404.1 185.3-54.7 18.2a57.4 57.4 0 0 1-27.1 2.4c5.1-7 12-13 20.9-17.5l51.5-25.8a57.7 57.7 0 1 0-77.4-77.4l-25.7 51.6a57.4 57.4 0 0 1-17.5 21 57.4 57.4 0 0 1 2.4-27.2l18.2-54.7a57.7 57.7 0 1 0-109.4 0l18.2 54.7a57.4 57.4 0 0 1 2.4 27.1 57.3 57.3 0 0 1-17.5-20.8l-25.8-51.6a57.7 57.7 0 1 0-77.4 77.4l51.6 25.7a57.4 57.4 0 0 1 21 17.5c-8.7 1.4-18 .7-27.2-2.4l-54.7-18.2a57.7 57.7 0 1 0 0 109.4l54.7-18.2a57.4 57.4 0 0 1 27.1-2.4 57.3 57.3 0 0 1-20.8 17.5l-51.6 25.8a57.7 57.7 0 1 0 77.4 77.4l25.7-51.6a57.4 57.4 0 0 1 17.5-21c1.4 8.7.7 18-2.4 27.2l-18.2 54.7a57.7 57.7 0 1 0 109.4 0l-18.2-54.7a57.4 57.4 0 0 1-2.4-27.1c7 5.1 13 12 17.5 20.9l25.8 51.5a57.7 57.7 0 1 0 77.4-77.4l-51.6-25.7a57.4 57.4 0 0 1-21-17.5c8.7-1.4 18-.7 27.2 2.4l54.7 18.2a57.7 57.7 0 1 0 0-109.4Z",
    fill: "#b8c1ff",
    border: "#7d8aff",
    pattern: {
      type: "dots",
      color: "#d2d8ff",
      size: 12,
    },
    name: "Flower",
  },
  {
    path: "M480 240a240 240 0 0 0-240 240 240 240 0 0 0 240-240ZM240 0A240 240 0 0 0 0 240 240 240 0 0 0 240 0ZM480 240A240 240 0 0 0 240 0a240 240 0 0 0 240 240ZM240 480A240 240 0 0 0 0 240a240 240 0 0 0 240 240Z",
    fill: "#ffb7b2", 
    border: "#ff8c8c",
    pattern: {
      type: "grid",
      color: "#ffcbca",
      size: 14,
    },
    name: "Star",
  },
]; 